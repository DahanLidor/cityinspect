...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(Enum(*USER_ROLES, name="user_role", create_type=False), default="inspector")
...
class Incident(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    hazard_type: Mapped[str] = mapped_column(Enum(*HAZARD_TYPES, name="hazard_type", create_type=False), nullable=False)
    severity: Mapped[str] = mapped_column(Enum(*SEVERITIES, name="incident_severity", create_type=False), default="medium")
    status: Mapped[str] = mapped_column(Enum(*STATUSES, name="incident_status", create_type=False), default="reported")
...
class IncidentCluster(Base):
    ...
    centroid: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
